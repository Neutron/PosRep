 ...
 **/
/* global resources, PosRep, Cesium, scene, angular, stctl, viewer, usrctl,$scope,DbService, $q, SocketService, GeoService, DlgBx*/ // ***** CONTROLLERS ******//

//viewCtl handles the interface
PosRep.controller('viewCtl', function() {
    var vwctl = this;
...
PosRep.controller('userCtl', function($scope, $http, $mdDialog, DbService, GeoService, SocketService, DlgBx) {
    var usrctl = this;
    usrctl.endpoint = {};
    usrctl.newuserid = "";
    usrctl.seluser = "";
    ...
    usrctl.registerEndpoint = function(data) {
        ...
        DbService.getKeys('User', function(kys) {
            if (kys === null) {
                ...
                DlgBx.custom("views/newuser.html").then(function(uname) {
                    if (uname === "New Unit") {
                        usrctl.newUser(data);
                    }
                    else {
    ...
    usrctl.newUnitUser = function(data) {
        DlgBx.prompt("New Node: ", "Enter Unit Name: ", data["socketid"].substring(4, 10), 'Node Name').then(function(uname) {
            usrctl.endpoint = {
                socketid: $scope.socketID,
...
PosRep.controller('mapCtl', function($scope, DbService, GeoService, SocketService, MsgService, DlgBx, $http) {
    var mapctl = this;
    ...
    mapctl.clearDb = function() {
        ...
        DlgBx.alert("Warning", "Confirm Deletion of Local Data", "Confirm Delete").then(function() {
    ...
    mapctl.addTrack = function(lat, lng) {
        ...
        DlgBx.prompt("No Track Selected", "Enter Name to Create Track", "New Track", "Track name").then(function(trackname) {
                var overwrite = null;
                ...
                if (overwrite !== null) {
                    DlgBx.alert("Duplicate Name", "Track name exists", "").then(function(resp) {
                        console.log(resp)
                    });
                }
                else {
        ...
        mapctl.removeGeoFence = function(indx) {
            DlgBx.confirm("Delete GeoFence " + mapctl.geofenceselected._name, "This cannot be undone", "Confirm", "Cancel").then(function(yes) {
                if (yes) {
...
PosRep.controller('messageCtl', function(GeoService, SocketService, MsgService, $http, $scope) {
    var msgctl = this;
    msgctl.addmap = false;
    msgctl.messages = [];
    msgctl.reports = [];
            ...
            $http.get('json/marcims.json').success(function(data) {
                ...
                for (var r in data.results) {
                    ...
                    if (typeof rptsel === 'undefined') {
                        ...
                        var report = msgctl.makeRpt(data.results[r]);
                        MsgService.postMsg('/udpmsg/', {
                            scktid: SocketService.scktid,
                            scktmsg: "MESSAGE",
                            payload: {
    ...
    msgctl.addRpt = function(rpt) {
        var report = msgctl.makeRpt(rpt);
        //console.log(report);
        msgctl.reports.push(report);
        GeoService.sdatasources[GeoService.mapid].entities.add({
            id: report._id,
            name: report._name,
            position: Cesium.Cartesian3.fromDegrees(report._location[1], report._location[0]),
            point: {
                pixelSize: 5,
    ...
    SocketService.socket.on('MESSAGE', function(msg) {
        var rptid = msg.payload.unit_id;
        console.log("POSREP Received for " + rptid + " at " + Date.now());
        msgctl.mapid = GeoService.mapid;
        var entity = GeoService.sdatasources[msgctl.mapid].entities.getById(rptid);
        if (typeof entity !== 'undefined') {
            if (msg.scktid === SocketService.scktid) {
                msgctl.addRpt(msg.payload.report);
            }
        }
    });
});
...
//
PosRep.directive('draggable', ['$window', function($window) {
    return {
        restrict: 'A',
...
//
PosRep.directive('draghandle', ['$window', function($window) {
    return {
        restrict: 'A',
        link: function(scope, elm, attrs) {
            var startX, startY, initialMouseX, initialMouseY;
