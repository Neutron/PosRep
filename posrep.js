...
(function() {
    ...
    var server = app.listen(server_port, argv.public ? undefined : server_ip_address, function() {
        if (argv.public) {
            console.log('PosRep development server running publicly.  Connect to http://localhost:%d/', server.address().port);
        }
        else if (argv.publicssl) {
            server.key = fs.readFileSync('key.pem');
            server.cert = fs.readFileSync('cert.pem');
            console.log('PosRep development server running publicly.  Connect to https://localhost:%d/', server.address().port);
        }
        else {
            console.log('PosRep development server running locally.  Connect to http://localhost:%d/', server.address().port);
        }
    });
